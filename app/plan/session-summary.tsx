...
function fmtDuration(totalSeconds: number, t: (k: TKey) => string): string {
  const m = Math.round(totalSeconds / 60);
  if (m < 60) return `${m} min`;
  return `${Math.floor(m / 60)}${t('hour_short')} ${m % 60}${t('min_short')}`;
}

function prLabel(recordType: PersonalRecordType, weightKg: number, reps: number, estimatedOneRm: number, t: (k: TKey) => string): string {
  if (recordType === 'max_weight') return fill(t('pr_max_weight'), { kg: weightKg, reps });
  if (recordType === 'max_1rm') return fill(t('pr_max_1rm'), { kg: Math.round(estimatedOneRm) });
  return fill(t('pr_max_reps'), { kg: weightKg, reps });
}
...
export default function SessionSummaryScreen() {
  ...
  const [routineName, setRoutineName] = useState(summary?.session.name ?? t('my_routine'));
  ...
  const handleSaveRoutine = async () => {
    ...
    try {
      ...
      Alert.alert(t('oops'), e.message ?? t('routine_save_failed'));
    } finally {
      setSaving(false);
    }
  };
  ...
  if (!summary) {
    return (
      <View style={{ flex: 1, backgroundColor: c.bg, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 24 }}>
        <Text style={{ fontSize: 15, color: c.sub, textAlign: 'center', marginBottom: 16 }}>{t('workout_saved')}</Text>
        <Button label={t('done')} onPress={handleDone} />
      </View>
    );
  }
    ...
    <View style={{ flex: 1, backgroundColor: c.bg }}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingTop: insets.top + 24, paddingHorizontal: 16, paddingBottom: insets.bottom + 120 }}>
        <View style={{ alignItems: 'center', marginBottom: 22 }}>
          ...
          <Text style={{ fontSize: 24, fontWeight: '800', color: c.text, letterSpacing: -0.5 }}>{t('workout_complete')}</Text>
        ...
        <View style={{ flexDirection: 'row', gap: 10, marginBottom: 16 }}>
          <Card pad={14} style={{ flex: 1, alignItems: 'center' }}>
            <Text style={{ fontSize: 11, color: c.sub, fontWeight: '600', marginBottom: 6 }}>{t('duration_caps')}</Text>
            <Text style={{ fontSize: 17, fontWeight: '800', color: c.text }}>{fmtDuration(summary.durationSeconds, t)}</Text>
          </Card>
          <Card pad={14} style={{ flex: 1, alignItems: 'center' }}>
            <Text style={{ fontSize: 11, color: c.sub, fontWeight: '600', marginBottom: 6 }}>{t('volume_caps')}</Text>
            <Text style={{ fontSize: 17, fontWeight: '800', color: c.text }}>{Math.round(summary.totalVolumeKg).toLocaleString(locale)} kg</Text>
          </Card>
          <Card pad={14} style={{ flex: 1, alignItems: 'center' }}>
            <Text style={{ fontSize: 11, color: c.sub, fontWeight: '600', marginBottom: 6 }}>{t('sets_caps')}</Text>
        ...
        {summary.personalRecords.length > 0 ? (
          <>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 7, marginBottom: 12, marginHorizontal: 2 }}>
              ...
              <Text style={{ fontSize: 15, fontWeight: '700', color: c.text }}>{t('personal_records')}</Text>
            </View>
              ...
              {summary.personalRecords.map((pr, i) => (
                <Card key={`${pr.exerciseId}-${pr.recordType}-${i}`} pad={13} accent style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={{ fontSize: 12, color: c.sub, marginTop: 1 }}>{prLabel(pr.recordType, pr.weightKg, pr.reps, pr.estimatedOneRm, t)}</Text>
                  </View>
                </Card>
              ))}
            </View>
        ...
        {routineExercises.length > 0 ? (
          saved ? (
            <Card pad={14} style={{ marginBottom: 8, flexDirection: 'row', alignItems: 'center', gap: 10 }}>
              ...
              <Text style={{ fontSize: 13, color: c.text, fontWeight: '600' }}>{t('saved_as_routine')}</Text>
            </Card>
          ) : showSaveRoutine ? (
            <Card pad={14} style={{ marginBottom: 8, gap: 10 }}>
              <Input label={t('routine_name')} value={routineName} onChangeText={setRoutineName} placeholder={t('my_routine')} />
              <Button label={saving ? t('saving') : t('save_routine')} onPress={handleSaveRoutine} loading={saving} icon="flag" />
            </Card>
          ) : (
            <TouchableOpacity activeOpacity={0.7} onPress={() => setShowSaveRoutine(true)} style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, borderWidth: 1, borderColor: c.line, borderRadius: 14, paddingVertical: 13, marginBottom: 8 }}>
              ...
              <Text style={{ fontSize: 13.5, fontWeight: '700', color: c.text }}>{t('save_as_routine')}</Text>
            </TouchableOpacity>
          )
        ) : null}
      </ScrollView>
